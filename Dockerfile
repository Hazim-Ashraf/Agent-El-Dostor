...
# MPS/CUDA passthrough), and on x86_64 the default PyPI torch pulls ~2.5 GB of NVIDIA CUDA
# libraries — which also overflows a small disk. Forcing the CPU wheel keeps the image
# ~2.5 GB smaller. On arm64 (M1) PyPI's torch is already CPU-only.
...
# torch is already satisfied above, so it isn't re-pulled (no CUDA). The pip cache mount
# downloads ONLY new wheels on a dependency change.
COPY pyproject.toml ./
RUN --mount=type=cache,target=/root/.cache/pip \
