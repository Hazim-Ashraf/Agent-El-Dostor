...
# Install CPU-only PyTorch + torchvision FIRST. This project never uses a GPU (Docker on
# macOS has no MPS/CUDA passthrough), and on x86_64 the default PyPI torch/torchvision pull
# ~2.5 GB of NVIDIA CUDA libraries — which also overflows a small disk. Forcing the CPU
# wheels keeps the image ~2.5 GB smaller. On arm64 (M1) PyPI's builds are already CPU-only.
...
# The rest of the Python deps, keyed ONLY on pyproject.toml (so code edits don't reinstall).
# torch/torchvision are already satisfied above, so they aren't re-pulled (no CUDA). The pip
# cache mount downloads ONLY new wheels on a dependency change.
COPY pyproject.toml ./
RUN --mount=type=cache,target=/root/.cache/pip \
